...
public class AbstractServiceTest extends TestCase {
    ...
    private static class NoOpService extends AbstractService {
        ...
        public void notifyFaile(Throwable cause) {
    ...
    public void testManualServiceStartStop() throws Exception {
        ...
        service.notifyStarte(); // usually this would be invoked by another thread
        assertEquals(State.RUNNING, service.state());
        assertTrue(service.isRunning());
        ...
        service.notifyStoppe(); // usually this would be invoked by another thread
        assertEquals(State.TERMINATED, service.state());
        assertFalse(service.isRunning());
        assertEquals(
                ImmutableList.of(
    ...
    public void testManualServiceNotifyStoppedWhileRunning() throws Exception {
        ...
        service.startAsync();
        service.notifyStarte();
        service.notifyStoppe();
        assertEquals(State.TERMINATED, service.state());
        assertFalse(service.isRunning());
    ...
    private static class ManualSwitchedService extends AbstractService {
        ...
        public void notifyStoppe() {
            super.notifyStopped();
        }

        public void notifyStarte() {
            super.notifyStarted();
        }
    ...
    public void testManualServiceFailureIdempotence() {
        ...
        service.startAsync();
        service.notifyFaile(new Exception("1"));
        service.notifyFaile(new Exception("2"));
    ...
    public void testNotifyFailedWhenNotStarted() {
        DefaultService service = new DefaultService();
        try {
            service.notifyFaile(new Exception());
            fail();
        } catch (IllegalStateException expected) {
        }
    }
